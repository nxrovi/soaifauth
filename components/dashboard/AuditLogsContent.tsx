...
export const AuditLogsContent: React.FC<AuditLogsContentProps> = ({
  user,
  apps,
  ...
  const handleExport = async () => {
    ...
    try {
      const res = await fetch(`/api/audit-logs/export?appId=${selectedAppId}`, {
        method: 'GET',
      })
      ...
      toast({
        ...
        description: 'The audit logs have been exported successfully.',
      })
    } catch (e: any) {
      toast({
        variant: 'error',
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data?.error || 'Failed to delete audit logs')
      ...
      toast({
        variant: 'error',
        title: 'Failed to delete audit logs',
        description: e?.message || 'Something went wrong.',
      })
    }
  }
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="glass-card rounded-2xl p-4 sm:p-6 border border-black/5 dark:border-white/5">
            ...
            <p className="text-xs text-black/60 dark:text-white/60">
              Monitor the activity of your Resellers and Managers.{' '}
              <a
                href="https://keyauthdocs.apidog.io/dashboard/app/audit-logs"
                target="_blank"
                ...
                <tbody className="divide-y divide-black/5 dark:divide-white/5">
                  {paginatedAuditLogs.length === 0 ? (
                    ...
                    paginatedAuditLogs.map((log) => (
                      ...
                      >
                        <td className="px-4 py-3 text-black/70 dark:text-white/70">{log.user}</td>
                        <td className="px-4 py-3 text-black/70 dark:text-white/70">{log.event}</td>
                        <td className="px-4 py-3 text-black/70 dark:text-white/70">{formatDate(log.time)}</td>
                      </tr>
                    ))
                  )}
                </tbody>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-black/60 dark:text-white/60">
                  Showing {filteredAuditLogs.length === 0 ? 0 : (currentPage - 1) * pageSize + 1} to{' '}
                  {Math.min(currentPage * pageSize, filteredAuditLogs.length)} of {filteredAuditLogs.length} records
                </div>
                <div className="flex items-center gap-2">
                  <button
      ...
      <DeleteModal
        ...
        message="Are you sure you want to delete all of your Audit Logs? This cannot be undone."
      />
    </>
  )
}
