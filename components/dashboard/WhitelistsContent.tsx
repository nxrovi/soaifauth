...
interface WhitelistsContentProps {
  user: {
    name?: string | null
    email: string
  }
...
export const WhitelistsContent: React.FC<WhitelistsContentProps> = ({
  user,
  apps,
  whitelists: initialWhitelists,
  currentAppId,
}) => {
  const router = useRouter()
  const { toast } = useToast()
  const [whitelists, setWhitelists] = useState<Whitelist[]>(initialWhitelists)
  ...
  const handleAppChange = async (appId: string) => {
    ...
    try {
      ...
      router.push('/dashboard/whitelists')
      router.refresh()
    } catch (e: any) {
      toast({
        variant: 'error',
  ...
  const handleCreate = async () => {
    if (!selectedAppId || !createForm.ip || !createForm.reason) {
      toast({
        variant: 'error',
        title: 'Missing fields',
        description: 'Please provide an IP address and reason.',
      })
      return
    }
    ...
    try {
      ...
      if (!res.ok) throw new Error(data?.error || 'Failed to create whitelist')

      toast({
        variant: 'success',
        title: 'Whitelist created',
        description: 'The whitelist entry has been created successfully.',
      })

      setShowCreateModal(false)
      setCreateForm({ ip: '', reason: '' })
      router.refresh()
    } catch (e: any) {
      toast({
        variant: 'error',
        title: 'Failed to create whitelist',
        description: e?.message || 'Something went wrong.',
      })
    }
  }

  const handleDelete = async (whitelistId?: string) => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data?.error || 'Failed to delete whitelist')

      toast({
        variant: 'success',
        title: 'Whitelist deleted',
        description: whitelistId
          ? 'The whitelist entry has been deleted successfully.'
          : 'All whitelists have been deleted successfully.',
      })
      ...
      toast({
        variant: 'error',
        title: 'Failed to delete whitelist',
        description: e?.message || 'Something went wrong.',
      })
    }
  }
  ...
  // Filter and paginate whitelists
  const filteredWhitelists = useMemo(() => {
    let filtered = whitelists

    if (searchQuery) {
      filtered = filtered.filter(
        (whitelist) =>
          whitelist.id.toLowerCase().includes(searchQuery.toLowerCase()) ||
          whitelist.ip.toLowerCase().includes(searchQuery.toLowerCase()) ||
          whitelist.reason.toLowerCase().includes(searchQuery.toLowerCase())
      )
    }

    return filtered
  }, [whitelists, searchQuery])

  const paginatedWhitelists = useMemo(() => {
    const start = (currentPage - 1) * pageSize
    const end = start + pageSize
    return filteredWhitelists.slice(start, end)
  }, [filteredWhitelists, currentPage, pageSize])
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      Object.entries(actionDropdownRefs.current).forEach(([whitelistId, ref]) => {
        if (ref && !ref.contains(event.target as Node)) {
          if (openActionDropdown === whitelistId) {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="glass-card rounded-2xl p-4 sm:p-6 border border-black/5 dark:border-white/5">
            ...
            <p className="text-xs text-black/60 dark:text-white/60">
              ...
              <a
                href="https://keyauthdocs.apidog.io/dashboard/app/whitelists"
                target="_blank"
                  ...
                  {paginatedWhitelists.length === 0 ? (
                    <tr>
                      <td colSpan={3} className="px-4 py-8 text-center text-black/60 dark:text-white/60">
                        {searchQuery ? 'No whitelists found matching your search.' : 'No whitelists found. Create your first whitelist to get started.'}
                      </td>
                    </tr>
                  ) : (
                    paginatedWhitelists.map((whitelist) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="relative" ref={(el) => { actionDropdownRefs.current[whitelist.id] = el }}>
                            <button
                              onClick={() => setOpenActionDropdown(openActionDropdown === whitelist.id ? null : whitelist.id)}
                            ...
                            >
                              ...
                              <svg
                                className={`fill-current h-4 w-4 transition-transform duration-200 ${
                                  openActionDropdown === whitelist.id ? 'rotate-180' : ''
                                }`}
                            ...
                            {openActionDropdown === whitelist.id && (
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-black/60 dark:text-white/60">
                  Showing {filteredWhitelists.length === 0 ? 0 : (currentPage - 1) * pageSize + 1} to{' '}
                  {Math.min(currentPage * pageSize, filteredWhitelists.length)} of{' '}
                  {filteredWhitelists.length} records
                </div>
                <div className="flex items-center gap-2">
                  <button
      ...
      <DeleteModal
        ...
        loading={false}
        title="Delete All Whitelists"
        message="Are you sure you want to delete all of your whitelists? This cannot be undone."
      />
    </>
  )
}
...
const CreateWhitelistModal: React.FC<{
  ...
  <Modal open={open} onClose={onClose} title="Add Whitelist">
    <form
      className="space-y-4"
      onSubmit={(e) => {
        e.preventDefault()
    ...
    >
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <Button type="submit" disabled={!form.ip || !form.reason}>
          Whitelist
        </Button>
      </div>
    </form>
